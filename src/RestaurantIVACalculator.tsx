...
export default function RestaurantIVACalculator() {
  ...
  const montoNumerico = useMemo(() => evaluarExpresion(montoExpresion), [montoExpresion]);
  ...
  const montoDescontadoFactura = tipoDescuento === 'factura' 
  ...
  const montoPOS = montoPOSManual !== '' 
    ? (parseFloat(montoPOSManual) || 0)
    : montoPOSCalculado;
  ...
  }, [tipoPropina, propinaPorcentaje, propinaFija, montoNumerico, tipoDescuento, montoPOS]);
  ...
  const montoDescuentoTarjeta = tipoDescuento === 'reembolso' 
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 text-lg font-semibold bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:outline-none transition-all focus:border-orange-500 focus:ring-orange-500/20"
              />
            </div>
            {expresionTieneOperador && montoNumerico > 0 && (
              <p className="text-xs text-slate-400 mt-1">= $ {formatMoney(montoNumerico)}</p>
            )}
            <p className="text-xs text-slate-500 mt-1">Podés sumar: 500+300 o restar: 1000-200</p>
          </div>
          ...
          <div>
              ...
              <div className="relative">
                ...
                <input
                  type="number"
                  value={tipoPropina === 'porcentaje' ? propinaPorcentaje : propinaFija}
                  onChange={(e) => tipoPropina === 'porcentaje' 
                    ? setPropinaPorcentaje(e.target.value) 
                    : setPropinaFija(e.target.value)
                  }
                  placeholder={tipoPropina === 'porcentaje' ? '10' : '0.00'}
                  className={`w-full py-3 text-lg font-semibold bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-500 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 focus:outline-none transition-all ${
              ...
              {tipoPropina === 'porcentaje' && propinaNumerico > 0 && (
                <p className="text-xs text-slate-400 mt-1">= $ {formatMoney(propinaNumerico)}</p>
              )}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                min="0"
                max="22"
                className="w-full pl-4 pr-10 py-3 text-lg font-semibold bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:outline-none transition-all focus:border-orange-500 focus:ring-orange-500/20"
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 font-medium">%</span>
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">
              Descuento de tarjeta <span className="text-slate-500">(opcional)</span>
            </label>
            <div className="relative">
              <input
                type="number"
            ...
            <p className="text-xs text-slate-500 mt-1">Ej: Scotiabank 25%, Itaú 20%, etc.</p>
            ...
            {tipoDescuento === 'reembolso' && descuentoPorcentaje > 0 && (
              ...
              >
                <div className={`relative w-11 h-6 rounded-full transition-colors duration-300 ${
                  propinaEnDescuento ? 'bg-orange-500' : 'bg-white/20'
                }`}>
                ...
                <span className="text-xs text-slate-400">Incluir propina en el descuento</span>
              </button>
            )}
          </div>
        </div>
        ...
        {hayResultados && (
          <div className="space-y-2">
            <div className="flex justify-between items-center py-2 text-sm">
              ...
              <span className="text-slate-300">$ {formatMoney(montoNumerico)}</span>
            </div>

            {tipoDescuento === 'factura' && descuentoPorcentaje > 0 && (
              <>
                <div className="flex justify-between items-center py-2 text-sm">
                  <span className="text-blue-400">Dto. en factura ({descuentoPorcentaje}%)</span>
                  <span className="text-blue-400">- $ {formatMoney(montoDescuentoTarjeta)}</span>
                </div>
                <div className="flex justify-between items-center py-2 text-sm border-b border-white/10">
                  ...
                  <span className="text-slate-300">$ {formatMoney(montoDescontadoFactura)}</span>
                </div>
                <div className="flex justify-between items-center py-2 text-sm">
                  <div className="flex-1">
                    <span className="text-slate-400">Monto en POS</span>
                    <p className="text-xs text-slate-500">Editá si redondearon</p>
                  </div>
            ...
            {propinaNumerico > 0 && (
              <div className="flex justify-between items-center py-2 text-sm">
                <div>
                  <span className="text-slate-400">Propina {tipoPropina === 'porcentaje' ? `(${propinaPorcentaje}%)` : ''}</span>
                  {tipoPropina === 'porcentaje' && montoPOSManual !== '' && (
                    <p className="text-xs text-slate-500">Sobre POS $ {formatMoney(montoPOS)}</p>
                  )}
                </div>
                <span className="text-slate-300">$ {formatMoney(propinaNumerico)}</span>
              </div>
            )}
            ...
            {tipoDescuento === 'reembolso' && descuentoPorcentaje > 0 && (
              <div className="flex justify-between items-center py-2 text-sm">
                <div>
                  <span className="text-blue-400">Dto. tarjeta ({descuentoPorcentaje}%)</span>
                  {propinaNumerico > 0 && (
                    <p className="text-xs text-slate-500">
                      {propinaEnDescuento ? 'Sobre cuenta + propina' : 'Solo sobre cuenta'}
                    </p>
                  )}
                </div>
                <span className="text-blue-400">- $ {formatMoney(montoDescuentoTarjeta)}</span>
            ...
            {ivaReembolso > 0 && (
              <div className="flex justify-between items-center py-2 text-sm">
                <div>
                  <span className="text-orange-400">Devolución IVA ({ivaReembolso}%)</span>
                  <p className="text-xs text-slate-500">
                    Sobre gravado $ {formatMoney(montoGravado)}
                    {montoPOSManual !== '' && ' (POS)'}
                  </p>
                </div>
                <span className="text-orange-400">- $ {formatMoney(descuentoIVA)}</span>
              </div>
            )}

            <div className="rounded-2xl p-4 mt-3 bg-gradient-to-r from-orange-500 to-red-500">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm text-orange-100">Pagás</p>
                  <p className="text-white text-2xl font-bold">$ {formatMoney(precioFinal)}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-orange-100">Ahorrás</p>
                  <p className="text-white text-lg font-semibold">$ {formatMoney(ahorroTotal)}</p>
                  <p className="text-xs text-orange-100">({porcentajeAhorro.toFixed(1)}%)</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="rounded-xl border overflow-hidden border-orange-500/30 bg-orange-950/20">
                <button
                  onClick={() => setAcordeonAbierto(acordeonAbierto === 'factura' ? null : 'factura')}
                ...
                {acordeonAbierto === 'factura' && (
                  <div className="px-3 pb-3 space-y-2 text-sm border-t border-white/10 pt-3">
                    ...
                    {tipoDescuento === 'factura' && descuentoPorcentaje > 0 && (
                      <div className="flex justify-between">
                        <span className="text-blue-400">Descuento ({descuentoPorcentaje}%)</span>
                        <span className="text-blue-400">- $ {formatMoney(montoNumerico * descuentoPorcentaje / 100)}</span>
                      </div>
                    )}
                    <div className="border-t border-white/10 pt-2 mt-2">
                      <div className="flex justify-between">
                        <span className="text-slate-400">Subtotal gravado (22%)</span>
                        <span className="text-slate-300">$ {formatMoney(montoGravado)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">IVA 22%</span>
                        <span className="text-slate-300">$ {formatMoney(montoGravado * 0.22)}</span>
                      </div>
                    </div>
                    <div className="border-t border-white/10 pt-2 mt-2">
                      <div className="flex justify-between font-semibold">
                        <span className="text-white">Total facturado</span>
                        <span className="text-white">$ {formatMoney(tipoDescuento === 'factura' ? montoPOS : montoNumerico)}</span>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="rounded-xl border overflow-hidden border-orange-500/30 bg-orange-950/20">
                <button
                  onClick={() => setAcordeonAbierto(acordeonAbierto === 'pos' ? null : 'pos')}
                  className="w-full flex items-center justify-between p-3 text-left"
                >
                  <span className="flex items-center gap-2 text-sm font-medium text-white">
                ...
                {acordeonAbierto === 'pos' && (
                  <div className="px-3 pb-3 space-y-2 text-sm border-t border-white/10 pt-3">
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-300">$ {formatMoney(montoPOS)}</span>
                    </div>
                    {propinaNumerico > 0 && (
                      <div className="flex justify-between">
                    ...
                    <div className="border-t border-white/10 pt-2 mt-2">
                      <div className="flex justify-between font-semibold">
                        ...
                        <span className="text-white">$ {formatMoney(montoPOS + propinaNumerico - descuentoIVA)}</span>
                      </div>
                    </div>
                  </div>
                )}
